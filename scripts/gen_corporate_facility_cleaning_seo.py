#!/usr/bin/env python3
"""
Generator for /src/components/service-detail/corporate-facility-cleaning-seo.tsx
...
/services/corporate-facility-cleaning.
...
The user brief (Corporate Facility Cleaning Services Scotland semantic SEO)
was processed to:
  ...
  - Convert the two [Button: ...] markers from the brief to real CTA buttons:
      [Button: Book My Free Site Survey Today]              -> /quote
      [Button: View Our Commercial Cleaning Checklist]      -> #checklist

The 9 exported components are:
  1. CorporateFacilitySeoHeroBanner   (dark hero, H1 + CTA buttons)
  2. CorporateFacilitySeoIntro         (bg-background, split-layout)
  3. CorporateFacilitySeoWhy           (bg-cream, image + 4 trust cards)
  4. CorporateFacilitySeoChecklist     (bg-background, 3 image cards, id="checklist")
  5. CorporateFacilitySeoBooking       (bg-background, 3-step process)
  6. CorporateFacilitySeoInterlinking  (bg-cream, 4 interlink cards)
  7. CorporateFacilitySeoNearMe        (bg-background, split + 36-location grid)
  8. CorporateFacilitySeoFinalCta      (bg-brand-dark, two CTAs)
  9. CorporateFacilitySeoFaq           (bg-background, 6 Q&As)
"""
...
OUT = Path(
    ...
    "corporate-facility-cleaning-seo.tsx"
)


def main() -> None:
    """
    Emit the contents of corporate-facility-cleaning-seo.tsx to stdout.

    Usage:
        python3 gen_corporate_facility_cleaning_seo.py \
            > src/components/service-detail/corporate-facility-cleaning-seo.tsx
