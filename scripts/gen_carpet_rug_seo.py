#!/usr/bin/env python3
"""
Generator for /src/components/service-detail/carpet-rug-cleaning-seo.tsx
...
/services/carpet-rug-cleaning.

Design pattern matches maid-service-seo.tsx exactly:
  - Centered text intros with brand chip + font-extrabold headings
  - text-gradient-brand on stat values
  - Image+copy split with aspect-[4/3] rounded-3xl shadow-brand
  - In-image overlay caption with bg-white/95 backdrop-blur
  - 2+2 trust card layout with bg-[var(--brand-light)] icon bg
  - Booking section with connector line + circular icon design
  - 4-column horizontal interlink cards with aspect-[16/9] image headers
  - Near-me section with [1fr_1.3fr] split + white grid card
  - Final CTA with decorative blur orbs
  - FAQ with motion.div wrapping details + custom +/- icon
  - HTML entities for typography consistency
  - Pound sign (£) instead of "GBP"

The user brief (Carpet & Rug Cleaning semantic SEO) was processed to:
  - Strip SEO-strategy language: "Topical Authority", "Semantic Interlinking:",
    "MaundyClean Semantic SEO Strategy", "[H1]/[H2]/[H3]", "[Button: ...]"
  - Strip source-citation placeholders: "[Rug Cleaner Source]",
    "[Fastclean Source]", "[Blitzers Source]"
  - Strip competitor names: "Rug Cleaner Scotland", "Fastclean Scotland"
  - Convert bracketed link placeholders to real Next.js <Link> components:
      ...
      [Sofa Deep Cleaning]         -> /services/upholstery-cleaning
...
Output: /home/z/my-project/src/components/service-detail/carpet-rug-cleaning-seo.tsx
"""

from pathlib import Path

OUT_PATH = Path("/home/z/my-project/src/components/service-detail/carpet-rug-cleaning-seo.tsx")

# Use a raw triple-double-quoted string. The TSX content uses single quotes
# for HTML attributes / imports, so no escaping is needed.
TSX = r""""use client";
...
# Fix the typo introduced in the FAQ +/- icon (top:0 bottom:0 with colon instead of space)
TSX = TSX.replace(
    '<span className="absolute left-1/2 top-0 bottom:0 w-0.5 bg-[var(--brand)] -translate-x-1/2" />',
    '<span className="absolute left-1/2 top-0 bottom-0 w-0.5 bg-[var(--brand)] -translate-x-1/2" />',
)
...
def main() -> None:
    ...
    # a literal " char that needs to be removed. The first real char of the
    # file should be the " of "use client";.
    content = TSX
    if content.startswith('""use client'):
        # Strip the single extra leading " (the 4th quote of r"""")
        content = content[1:]
