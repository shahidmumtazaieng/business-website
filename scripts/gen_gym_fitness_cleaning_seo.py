#!/usr/bin/env python3
"""
Generator for /src/components/service-detail/gym-fitness-cleaning-seo.tsx
...
/services/gym-fitness-cleaning.

Design pattern matches cafe-coffee-shop-cleaning-seo.tsx exactly (the most
...
The user brief (Gym & Fitness Centre Cleaning Services Scotland semantic SEO)
was processed to:
  ...
  - Strip source citations: [Source ###], [Competitor Text], [Competitor Source]
    (numerous source markers present in the brief - all stripped)
  - Drop the "Semantic Interlinking:" prefix from the section title
    (kept as "Solutions for Every Property Stage" per the brief)
...
The 9 exported components are:
  1. GymFitnessSeoHeroBanner   (dark hero, H1 + CTA buttons)
  2. GymFitnessSeoIntro         (bg-background, split-layout, 2 inline Links in p2)
  3. GymFitnessSeoWhy           (bg-cream, image + 4 trust cards)
  4. GymFitnessSeoChecklist     (bg-background, 3 image cards, id="checklist")
  5. GymFitnessSeoBooking       (bg-background, 3-step process)
  6. GymFitnessSeoInterlinking  (bg-cream, 4 interlink cards)
  7. GymFitnessSeoNearMe        (bg-background, split + 36-location grid)
  8. GymFitnessSeoFinalCta      (bg-brand-dark, two CTAs)
  9. GymFitnessSeoFaq           (bg-background, 3 Q&As)
"""
...
OUT = Path(
    ...
    "gym-fitness-cleaning-seo.tsx"
)


def main() -> None:
    """
    Emit the contents of gym-fitness-cleaning-seo.tsx to stdout.

    Usage:
        python3 gen_gym_fitness_cleaning_seo.py \
            > src/components/service-detail/gym-fitness-cleaning-seo.tsx
