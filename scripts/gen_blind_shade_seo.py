#!/usr/bin/env python3
"""
Generator for /src/components/service-detail/blind-shade-cleaning-seo.tsx
...
/services/blind-shade-cleaning.

Design pattern matches housekeeping-services-seo.tsx exactly:
  Section background alternation:
    bg-background (Intro)
    bg-[var(--cream)] (Why)
    bg-background (Checklist)
    bg-background (Booking)
    bg-[var(--cream)] (Interlinking)
...
The user brief (Blind & Shade Cleaning semantic SEO) was processed to:
  - Strip SEO-strategy language: "Semantic Interlinking:",
    "[H1]/[H2]/[H3]", "[Button: ...]"
  - Strip source citations: none present
  - Strip competitor names: none present
  - Convert bracketed link placeholders to real Next.js <Link> components:
      ...
      [30-Second Online Calculator]-> /quote
  - Add a near-me section + standalone near-me page
"""

from pathlib import Path

OUT_PATH = Path("/home/z/my-project/src/components/service-detail/blind-shade-cleaning-seo.tsx")
...
print(f"Wrote {OUT_PATH} ({len(TSX):,} chars, {TSX.count(chr(10)) + 1} lines)")
