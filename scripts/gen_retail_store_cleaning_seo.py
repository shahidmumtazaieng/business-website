#!/usr/bin/env python3
"""
Generator for /src/components/service-detail/retail-store-cleaning-seo.tsx
...
/services/retail-store-cleaning.

Design pattern matches corporate-facility-cleaning-seo.tsx exactly (the most
...
The user brief (Retail Store Cleaning Services Scotland semantic SEO) was
processed to:
  ...
  - Drop the "Semantic Interlinking:" prefix from the section title
    (kept as "Commercial Solutions for Every Stage")
  - Convert bracketed link placeholders to real Next.js <Link> components:
      ...
      [Spring Cleaning Service]    -> /services/spring-cleaning
  ...
  - Convert the two [Button: ...] markers from the brief to real CTA buttons:
      [Button: Book Your Free Site Survey Today]      -> /quote
      [Button: View Our Retail Cleaning Checklist]    -> #checklist

The 9 exported components are:
  1. RetailStoreSeoHeroBanner   (dark hero, H1 + CTA buttons)
  2. RetailStoreSeoIntro         (bg-background, split-layout)
  3. RetailStoreSeoWhy           (bg-cream, image + 4 trust cards)
  4. RetailStoreSeoChecklist     (bg-background, 2 image cards, id="checklist")
  5. RetailStoreSeoBooking       (bg-background, 3-step process)
  6. RetailStoreSeoInterlinking  (bg-cream, 4 interlink cards)
  7. RetailStoreSeoNearMe        (bg-background, split + 36-location grid)
  8. RetailStoreSeoFinalCta      (bg-brand-dark, two CTAs)
  9. RetailStoreSeoFaq           (bg-background, 3 Q&As)
"""
...
OUT = Path(
    ...
    "retail-store-cleaning-seo.tsx"
)


def main() -> None:
    """
    Emit the contents of retail-store-cleaning-seo.tsx to stdout.

    Usage:
        python3 gen_retail_store_cleaning_seo.py \
            > src/components/service-detail/retail-store-cleaning-seo.tsx
