#!/usr/bin/env python3
"""
Generator for /src/components/service-detail/showroom-cleaning-seo.tsx
...
/services/showroom-cleaning.

Design pattern matches bank-financial-office-cleaning-seo.tsx exactly (the most
...
The user brief (Showroom Deep Cleaning Services Scotland semantic SEO) was
processed to:
...
The 9 exported components are:
  1. ShowroomSeoHeroBanner   (dark hero, H1 + CTA buttons)
  2. ShowroomSeoIntro         (bg-background, split-layout, 2 inline Links in p2)
  3. ShowroomSeoWhy           (bg-cream, image + 4 trust cards)
  4. ShowroomSeoChecklist     (bg-background, 3 image cards, id="checklist")
  5. ShowroomSeoBooking       (bg-background, 3-step process)
  6. ShowroomSeoInterlinking  (bg-cream, 4 interlink cards)
  7. ShowroomSeoNearMe        (bg-background, split + 36-location grid)
  8. ShowroomSeoFinalCta      (bg-brand-dark, two CTAs)
  9. ShowroomSeoFaq           (bg-background, 3 Q&As)
"""
...
OUT = Path(
    ...
    "showroom-cleaning-seo.tsx"
)


def main() -> None:
    """
    Emit the contents of showroom-cleaning-seo.tsx to stdout.

    Usage:
        python3 gen_showroom_cleaning_seo.py \
            > src/components/service-detail/showroom-cleaning-seo.tsx
