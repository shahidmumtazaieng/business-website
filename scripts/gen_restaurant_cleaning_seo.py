#!/usr/bin/env python3
"""
Generator for /src/components/service-detail/restaurant-cleaning-seo.tsx
...
Design pattern matches retail-store-cleaning-seo.tsx exactly (the most
...
The user brief (Restaurant Cleaning & Recurring Commercial Maintenance
Scotland semantic SEO) was processed to:
...
The 9 exported components are:
  1. RestaurantSeoHeroBanner   (dark hero, H1 + CTA buttons)
  2. RestaurantSeoIntro         (bg-background, split-layout)
  3. RestaurantSeoWhy           (bg-cream, image + 4 trust cards)
  4. RestaurantSeoChecklist     (bg-background, 3 image cards, id="checklist")
  5. RestaurantSeoBooking       (bg-background, 3-step process)
  6. RestaurantSeoInterlinking  (bg-cream, 4 interlink cards)
  7. RestaurantSeoNearMe        (bg-background, split + 36-location grid)
  8. RestaurantSeoFinalCta      (bg-brand-dark, two CTAs)
  9. RestaurantSeoFaq           (bg-background, 3 Q&As)
"""
...
OUT = Path(
    ...
    "restaurant-cleaning-seo.tsx"
)


def main() -> None:
    """
    Emit the contents of restaurant-cleaning-seo.tsx to stdout.

    Usage:
        python3 gen_restaurant_cleaning_seo.py \
            > src/components/service-detail/restaurant-cleaning-seo.tsx
