#!/usr/bin/env python3
"""
Generator for /src/components/service-detail/restroom-sanitation-seo.tsx
...
/services/restroom-sanitation.

Design pattern matches nightly-janitorial-seo.tsx exactly (the most
...
the booking heading - matches nightly-janitorial; DIFFERENT from day-porter
which used <h3> because day-porter's brief had an explicit [H3] marker;
restroom-sanitation brief has no [H3] marker, so use <h2> to match the
standard section-title pattern)
Interlinking section title uses <h2> (drops the "Semantic Interlinking:"
prefix per the brief - kept as "Hygiene Solutions for Every Stage")
...
The user brief (Restroom Sanitation & Washroom Cleaning Services Scotland
semantic SEO) was processed to:
  - Strip SEO-strategy language: "Topical Authority",
    "MaundyClean Semantic SEO Strategy", "Semantic Interlinking:",
    "[H1]/[H2]/[H3]", "[Button: ...]"
  - Strip source citations: [Competitor Source], [378, Competitor Source],
    [363, Competitor Source], [369, Competitor Source],
    [Competitor Source, 316], [Competitor Source, 373],
    [299, Competitor Source], [Source ###] (any source markers present in
    the brief - all stripped)
  - Drop the "Semantic Interlinking:" prefix from the section title
    (kept as "Hygiene Solutions for Every Stage" per the brief)
  ...
  - Convert the two [Button: ...] markers from the brief to real CTA buttons:
      [Button: Request Your Bespoke Hygiene Quote Today]   -> /quote
      [Button: View Our Bathroom Deep Cleaning Checklist]  -> #checklist

The 9 exported components are:
  1. RestroomSanitationSeoHeroBanner   (dark hero, H1 + CTA buttons)
  2. RestroomSanitationSeoIntro        (bg-background, split-layout,
                                        ONE inline Link in p2 to
                                        /services/recurring-home-cleaning per
                                        the brief - p2 is a JSX ReactNode
                                        paragraph not a string)
  3. RestroomSanitationSeoWhy          (bg-cream, image + 4 trust pillars:
                                        UserCheck / Droplets / ShieldCheck / Leaf;
                                        ALL 4 pillar bodies start with the
                                        brief sentence verbatim and are
                                        extended with restroom-sanitation-specific
                                        prose; typed as ReactNode for codebase
                                        consistency)
  4. RestroomSanitationSeoChecklist    (bg-background, 3 image cards,
                                        id="checklist", card icons
                                        HeartPulse / Droplets / ClipboardCheck;
                                        ALL sub-point texts in ALL 3 cards are
                                        plain strings per the brief but typed as
                                        ReactNode for codebase consistency)
  5. RestroomSanitationSeoBooking      (bg-background, 3-step process:
                                        ClipboardCheck / UserCheck / Sparkles;
                                        FileText imported for safety/template
                                        parity; H2 heading per brief (NO [H3]
                                        marker - matches nightly-janitorial;
                                        DIFFERENT from day-porter which used <h3>))
  6. RestroomSanitationSeoInterlinking (bg-cream, 4 interlink cards:
                                        KeyRound / Hammer / Sparkles / HeartPulse;
                                        ALL 4 card descriptions are ReactNode
                                        values; each contains an inline <Link>)
  7. RestroomSanitationSeoNearMe       (bg-background, split + 36-location grid;
                                        "Browse the near-me directory" button
                                        links to /services/restroom-sanitation-near-me#locations
                                        WITH the #locations anchor per the
                                        brief bug-warning - yields 37 anchored
                                        near-me links on the main page)
  8. RestroomSanitationSeoFinalCta     (bg-brand-dark, two CTAs)
  9. RestroomSanitationSeoFaq          (bg-background, 3 Q&As in brief order:
                                        provide own materials FIRST, then insured,
                                        then manage restocking of supplies - DO
                                        NOT reorder)
"""
...
OUT = Path(
    ...
    "restroom-sanitation-seo.tsx"
)


def main() -> None:
    """
    Emit the contents of restroom-sanitation-seo.tsx to stdout.

    Usage:
        python3 gen_restroom_sanitation_seo.py \
            > src/components/service-detail/restroom-sanitation-seo.tsx
